/*
 *   WildMidi MIDI music decoder based on the nonstream-audio GStreamer base class
...
static void gst_wildmidi_dec_set_property(GObject *object, guint prop_id, const GValue *value, GParamSpec *pspec);
static void gst_wildmidi_dec_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec);
...
static GstClockTime gst_wildmidi_dec_get_subsong_duration(GstNonstreamAudioDecoder *dec, guint subsong);

static guint gst_wildmidi_dec_get_supported_output_modes(GstNonstreamAudioDecoder *dec);
static gboolean gst_wildmidi_dec_decode(GstNonstreamAudioDecoder *dec, GstBuffer **buffer, guint *num_samples);
...
static void gst_wildmidi_shutdown_library(void)
{
	GST_DEBUG("WildMidi init instance counter: %lu", init_refcount);
...
void gst_wildmidi_dec_class_init(GstWildmidiDecClass *klass)
{
	GObjectClass *object_class;
	GstElementClass *element_class;
	GstNonstreamAudioDecoderClass *dec_class;

	GST_DEBUG_CATEGORY_INIT(wildmididec_debug, "wildmididec", 0, "WildMidi-based MIDI music decoder");
	...
	object_class->set_property = GST_DEBUG_FUNCPTR(gst_wildmidi_dec_set_property);
	object_class->get_property = GST_DEBUG_FUNCPTR(gst_wildmidi_dec_get_property);

	dec_class->tell                       = GST_DEBUG_FUNCPTR(gst_wildmidi_dec_tell);
	dec_class->seek                       = GST_DEBUG_FUNCPTR(gst_wildmidi_dec_seek);
	dec_class->load_from_buffer           = GST_DEBUG_FUNCPTR(gst_wildmidi_dec_load_from_buffer);
	dec_class->get_current_subsong        = GST_DEBUG_FUNCPTR(gst_wildmidi_dec_get_current_subsong);
	dec_class->get_num_subsongs           = GST_DEBUG_FUNCPTR(gst_wildmidi_dec_get_num_subsongs);
	dec_class->get_subsong_duration       = GST_DEBUG_FUNCPTR(gst_wildmidi_dec_get_subsong_duration);
	dec_class->get_supported_output_modes = GST_DEBUG_FUNCPTR(gst_wildmidi_dec_get_supported_output_modes);
...
void gst_wildmidi_dec_init(GstWildmidiDec *wildmidi_dec)
{
...
static gboolean gst_wildmidi_dec_seek(GstNonstreamAudioDecoder *dec, GstClockTime *new_position)
{
	...
	unsigned long int sample_pos = gst_util_uint64_scale_int(*new_position, WILDMIDI_SAMPLE_RATE, GST_SECOND);
	...
	*new_position = gst_util_uint64_scale_int(sample_pos, GST_SECOND, WILDMIDI_SAMPLE_RATE);
	return TRUE;
}
...
static GstClockTime gst_wildmidi_dec_tell(GstNonstreamAudioDecoder *dec)
{
	...
	return gst_util_uint64_scale_int(info->current_sample, GST_SECOND, WILDMIDI_SAMPLE_RATE);
}
...
static gboolean gst_wildmidi_dec_load_from_buffer(GstNonstreamAudioDecoder *dec, GstBuffer *source_data, G_GNUC_UNUSED guint initial_subsong, GstClockTime *initial_position, GstNonstreamAudioOutputMode *initial_output_mode, G_GNUC_UNUSED gint *initial_num_loops)
{
	...
	{
		GST_ERROR_OBJECT(wildmidi_dec, "Could not start loading: WildMidi is not initialized");
		return FALSE;
	}
	...
	{
		unsigned long int sample_pos = gst_util_uint64_scale_int(*initial_position, WILDMIDI_SAMPLE_RATE, GST_SECOND);
		WildMidi_FastSeek(wildmidi_dec->song, &sample_pos);
		*initial_position = gst_util_uint64_scale_int(sample_pos, GST_SECOND, WILDMIDI_SAMPLE_RATE);
	}
...
static guint gst_wildmidi_dec_get_current_subsong(G_GNUC_UNUSED GstNonstreamAudioDecoder *dec)
{
	return 0;
}


static guint gst_wildmidi_dec_get_num_subsongs(G_GNUC_UNUSED GstNonstreamAudioDecoder *dec)
{
	return 1;
}
...
static guint gst_wildmidi_dec_get_supported_output_modes(G_GNUC_UNUSED GstNonstreamAudioDecoder *dec)
{
...
static gboolean gst_wildmidi_dec_decode(GstNonstreamAudioDecoder *dec, GstBuffer **buffer, guint *num_samples)
{
	...
	decoded_size_in_bytes = WildMidi_GetOutput(wildmidi_dec->song, (char *)(info.data), info.size);
...
static void gst_wildmidi_dec_update_options(GstWildmidiDec *wildmidi_dec)
{
	...
	WildMidi_SetOption(wildmidi_dec->song, WM_MO_LOG_VOLUME | WM_MO_ENHANCED_RESAMPLING | WM_MO_REVERB, options);
}
